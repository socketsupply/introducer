module.exports = function ({ appname }, crypto, fs, path) {
  var appname = process.env.appname || 'introducer-chat'
  const config_file = path.join(process.env.HOME, '.' + appname)
  let config = {}
  ...
  if (!config.id) { config.id = crypto.randomBytes(32).toString('hex') }
