...
function createNatPeer (network, id, address_nat, address, Nat) {
  ...
  nat.add(address, new Node(peer = new Peer({ id, ...intros })))
  return [peer, nat]
}
...
test('nat-check', function (t) {
  ...
  var easy_node = easy_nat.add(d, new Node(peerD = new Peer({ id: ids.d, ...intros })))
  var hard_nat = new DependentNat()
  var hard_node = hard_nat.add(e, new Node(peerE = new Peer({ id: ids.e, ...intros })))
  var static_node = network.add(F, new Node(peerF = new Peer({ id: ids.f, ...intros })))
